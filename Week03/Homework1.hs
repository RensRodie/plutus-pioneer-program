...
mkValidator dat () ctx = traceIfFalse "Either wrong benif or deadline passed" beneficiary1Deadline ||
                         traceIfFalse "Either wrong benif or deadline not reached" beneficiary2Deadline
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    beneficiary1Deadline :: Bool
    beneficiary1Deadline = (txSignedBy info $ beneficiary1 dat) && (contains (to $ deadline dat) $ txInfoValidRange info)

    beneficiary2Deadline :: Bool
    beneficiary2Deadline = (txSignedBy info $ beneficiary2 dat) && (contains (from $ deadline dat) $ txInfoValidRange info) -- implement +1 to be precise (exclude deadline so there is no overlap)
