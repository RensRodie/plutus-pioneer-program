...
mkPolicy pkh deadline () ctx = traceIfFalse "wrong public key"   pubCheck           &&
                               traceIfFalse "deadline passed" deadlineCheck
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    pubCheck :: Bool
    pubCheck = txSignedBy info $ pkh

    deadlineCheck :: Bool
    deadlineCheck = contains (to $ deadline) $ txInfoValidRange info
